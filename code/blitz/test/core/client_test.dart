...
void main() {
  ...
  test(
      'Late events are sorted correctly: timestamp, then senderID, then eventID',
        ...
        inStreamCon.onListen = () async {
          ...
          expect(client.getUnstableEvents(), eventsExpected);
          inStreamCon.close();
        };
        client.init();
  ...
  test(
      'Events replace event with same ID (and, incorrectly, timestamp)', () async {
    inStreamCon.onListen = () async {
      currentTime = Duration(seconds: 3);
      final evtStrings = [
        't0-s0-e0-d0-local',
        't1000-s2-e3-d1-local',
  ...
  test(
      'Server and local-shared events are unstable, then baked when server event with timestamp is received',
        ...
        inStreamCon.onListen = () async {
          ...
          inStreamCon.add(makeEventCI('t750-r750-s1-e0-server'));
          await Future.delayed(Duration.zero);
          expect(client
              .getUnstableEvents()
              .length, 2);
          ...
          expect(client
              ...
              .length, 3);
          expect(client.getCurrentState(), 0);
          inStreamCon.add(makeEventCI('t2250-r2250-s1-e2-server'));
          await Future.delayed(Duration.zero);
          expect(client
              .getUnstableEvents()
              .length, 2);
          ...
          expect(client
              ...
              .length, 3);
          expect(client.getCurrentState(), 2);
          inStreamCon.add(makeEventCI('t3750-r3750-s1-e4-server'));
          await Future.delayed(Duration.zero);
          expect(client
              .getUnstableEvents()
              .length, 2);
  ...
  test(
      'Local events are only added to stable state if they receive server confirmation',
        ...
        inStreamCon.onListen = () async {
          ...
          inStreamCon.add(makeEventCI('t0-s0-e0-local'));
          inStreamCon.add(makeEventCI('t2000-r2000-s1-e0-server'));
          await Future.delayed(Duration.zero);
          expect(client
              .getUnstableEvents()
              .length, 1);
          expect(client.getCurrentState(), 0);
          inStreamCon.add(makeEventCI('t5000-s3-e0-local'));
          inStreamCon.add(makeEventCI('t5000-r5000-s3-e0-server'));
          inStreamCon.add(makeEventCI('t7000-r7000-s1-e2-server'));
          await Future.delayed(Duration.zero);
          expect(client
              .getUnstableEvents()
              .length, 1);
  ...
  test('Only local events are sent to the server', () async {
    inStreamCon.onListen = () async {
      ...
      inStreamCon.add(makeEventCI('t0-r0-s0-e1-d1-server'));
  ...
  test('getStateAt returns correct state in future at correct time', () async {
    inStreamCon.onListen = () async {
      ...
      inStreamCon.add(makeEventCI('t0-r0-s0-e0-server'));
      inStreamCon.add(makeEventCI('t100-r100-s0-e1-server'));
      inStreamCon.add(makeEventCI('t200-r200-s0-e2-server'));
      inStreamCon.add(makeEventCI('t3000-r3000-s0-e3-server'));
      await Future.delayed(Duration.zero);
