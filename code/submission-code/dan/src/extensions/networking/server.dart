...
class NetworkingServer {
  NetworkingServer({
    ...
    Duration artificialLatency = Duration.zero,
  })  : _clientConnectCallback = onClientConnect,
        _clientDisconnectCallback = onClientDisconnect,
        _port = port,
        _address = address,
        _generateUserID = generateUserID,
        _artificialLatency = artificialLatency {
    _dataToServer = _dataToServerGroup.stream.addDelay(_artificialLatency);
  }

  final String _address;
  final int _port;
  late final HttpServer _server;
  final Duration _artificialLatency;
  ...
  late final StreamGroup<(int clientID, String data)> _dataToServerGroup =
      StreamGroup();
  late final Stream<(int clientID, String data)> _dataToServer;
  ...
  void listenToDataForClients(Stream<(int, String)> stream) async {
    await for (final r in stream.addDelay(_artificialLatency)) {
  ...
  Stream<(int, String)> get dataToServer => _dataToServer;
}
