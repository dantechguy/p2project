...
Stream<EventClientInPreCore<OutData>> addTicks<InData, OutData>(
  ...
  final timerStream = streamPeriodic(tickPeriod).expand((tickNum) {
    final generatedTickEvents = _generateTickEventsUpTo(
      timestamp: getCurrentTime(),
      lastTickEventTimestamp: lastTickEventTimestamp,
      tickPeriod: tickPeriod,
      generateTickEvent: generateTickEvent,
    );
    if (generatedTickEvents.isNotEmpty) {
      lastTickEventTimestamp = generatedTickEvents.last.generatedTimestamp;
    }
    return generatedTickEvents;
  });
