...
EventClientInInCore<String> makeEventCIIn(String s) {
  if (s.endsWith('-local')) {
    return makeEventCIInLocal(s);
...
EventClientInFromLocalInCore<String> makeEventCIInLocal(String s) {
  ...
  return EventClientInFromLocalInCore(
...
EventClientInFromLocalPreCore<String> makeEventCIPreLocal(String s) {
  final pattern = RegExp(r'^d(\d+)?-local$');
  if (!pattern.hasMatch(s)) throw FormatException('invalid makeEvent string: $s');
